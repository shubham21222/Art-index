...
export default async function handler(req, res) {
  ...
  try {
    ...
    const collection = db.collection("eastern_european_artworks");
    ...
    const formattedArtworks = artworks.map((artwork) => ({
      ...
      image: {
        src: artwork.image?.resized?.src || artwork.image?.url || 'https://placehold.co/445x534?text=Eastern+European+Art',
    ...
    console.error("Error fetching Eastern European artworks from MongoDB:", error);
