...
export default async function handler(req, res) {
  ...
  try {
    ...
    const collection = db.collection("ceramics_artworks");
    ...
    const formattedArtworks = artworks.map((artwork) => ({
      ...
      image: {
        src: artwork.image?.resized?.src || artwork.image?.url || 'https://placehold.co/445x534?text=Ceramics',
    ...
    console.error("Error fetching Ceramics artworks from MongoDB:", error);
