...
export default async function handler(req, res) {
  ...
  try {
    ...
    const collection = db.collection("film_video_artworks");
    ...
    const formattedArtworks = artworks.map((artwork) => ({
      ...
      image: {
        src: artwork.image?.resized?.src || artwork.image?.url || 'https://placehold.co/445x534?text=New+Media+Video',
    ...
    console.error("Error fetching New Media and Video artworks from MongoDB:", error);
