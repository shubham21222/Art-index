...
const API_URL = "/api/middle-eastern-art";

export default function MiddleEasternArt() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching Middle Eastern artworks:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex flex-col items-center justify-center max-w-[1500px] mx-auto px-6 py-8">
      ...
      <div className="flex flex-col items-center justify-between w-full mb-6">
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-900">Middle Eastern Art</h2>
          <p className="text-gray-900 text-lg mt-2">Discover contemporary Middle Eastern art</p>
        </div>
        {/* <Link
          href="/middle-eastern-art"
          className="text-black text-sm font-medium hover:underline transition-colors duration-300 mt-4 md:mt-0"
        >
          View All Middle Eastern Art
        </Link> */}
      </div>
      ...
      <div className="relative h-[500px] w-full perspective-[1200px] overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center">
          {loading ? (
            ...
            <p className="text-gray-500 text-center">No Middle Eastern artworks found.</p>
          )}
        </div>
