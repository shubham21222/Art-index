  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPartners(estates || []);
        setCurrentIndex(Math.floor((estates || []).length / 2)); // Start at middle
      } catch (error) {
        console.error("Error fetching artist estates:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="px-6 py-8 max-w-[1500px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
        ...
        >
          Explore All Estates & Foundations
        </Link>
      </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {loading ? (
            ...
            partners.map((partner, index) => (
              ...
              >
                <Link href={`/estate/${partner.slug}`} className="block h-full">
                  {/* Image */}
