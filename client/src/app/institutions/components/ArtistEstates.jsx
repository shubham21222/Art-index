...
const API_URL = "/api/artist-estates";

export default function ArtistEstates() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching artist estates institutions:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex flex-col items-center justify-center max-w-[1500px] mx-auto px-6 py-8">
      ...
      <div className="flex flex-col items-center justify-between w-full mb-6">
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-900">Artist Estates & Foundations</h2>
          <p className="text-gray-900 text-lg mt-2">Discover leading artist estates and foundations</p>
        </div>
        <Link
          href="/institutions/artist-estates"
          className="text-black text-sm font-medium hover:underline transition-colors duration-300 mt-4 md:mt-0"
        >
          View All Estates & Foundations
        </Link>
      </div>
