...
const generateLotNumber = async () => {
    ...
    if (lastAuction && lastAuction.lotNumber) {
        const lotNumberParts = lastAuction.lotNumber.split("#");
        if (lotNumberParts.length > 1) {
            lastLotNumber = parseInt(lotNumberParts[1]) || 0;
        }
    }
...
// Generate sequential lot numbers for bulk operations
const generateSequentialLotNumbers = async (count) => {
    const lastAuction = await auctionModel.findOne().sort({ createdAt: -1 });
    let lastLotNumber = 0;
    ...
    for (let i = 1; i <= count; i++) {
        lotNumbers.push(`LOT#${lastLotNumber + i}`);
    }
...
export const createAuction = async (req, res) => {
    try {
        ...
        const auction = new auctionModel({
            internalID: `AUCTION_${Date.now()}`, // Add internalID
            product,
            category,
            startingBid,
...
export const deleteAuction = async (req, res) => {
    try {
        const { ids, auctionId } = req.body;

        // Handle both single auctionId and multiple ids
        let auctionIds = [];
        
        if (auctionId) {
            // Single auction deletion
            auctionIds = [auctionId];
        } else if (ids && ids.length) {
            // Multiple auctions deletion
            auctionIds = ids;
        } else {
            return badRequest(res, 'Please provide auction ID(s).');
        }

        const deletedAuctions = await auctionModel.deleteMany({ _id: { $in: auctionIds } });
        if (!deletedAuctions.deletedCount) {
            return notFound(res, 'Auctions not found.');
        }

        return success(res, `${deletedAuctions.deletedCount} auction(s) deleted successfully.`);
    } catch (error) {
...
export const createBulkAuction = async (req, res) => {
    try {
        ...
        if (!category) {
            return badRequest(res, "Missing category field.");
        }

        if (!stateDate) {
            return badRequest(res, "Missing startDate field.");
        }

        // Check if category exists by name, create if not found
        let categoryExists = await categoryModel.findOne({ name: { $regex: new RegExp(`^${category}$`, 'i') } });
        if (!categoryExists) {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (!product) {
                product = await AuctionProduct.create({
                    ...
                    lotNumber: productData.lotNumber || lotNumber,
                    status: "Not Sold",
                    favorite: false,
                    ...
                    auctionType: productData.auctionType || "TIMED",
                    count: 1
                });
            }

            // Push auction data to array with correct field names
            auctions.push({
                internalID: productData.internalID || productData.skuNumber || `AUCTION_${Date.now()}_${i}`, // Use provided internalID or SKU
                product: product._id, // Use 'product' not 'auctionProduct'
                category: categoryExists._id, // Use 'category' not 'auctioncategory'
                startingBid: product.sellPrice || productData.sellPrice || 0,
                currentBid: product.sellPrice || productData.sellPrice || 0,
                auctionType: product.auctionType || "TIMED",
                startDate: stateDate,
                endDate: endDate,
                lotNumber: lotNumber,
                description: desciptions || productData.description || "No description",
                createdBy: req.user._id,
                status: status || "ACTIVE",
